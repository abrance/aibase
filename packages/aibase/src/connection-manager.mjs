...
export class ConnectionManager {
  ...
  /**
   * @param {{ baseUrl: string, directory: string, username?: string, password?: string }} params
   */
  constructor({ baseUrl, directory, username, password }) {
    ...
    this.#eventSource = new EventSourceManager(
      { baseUrl: this.#candidateUrls[0], directory, username, password },
      this.#client,
    );
  ...
  #buildClient() {
    ...
    if (this.#password) {
      headers.authorization =
        "Basic " + Buffer.from(`${this.#username ?? "opencode"}:${this.#password}`).toString("base64");
    }
